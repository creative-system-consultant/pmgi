...
class PegawaiPemudahCara extends Component
{
    ...
    protected $listeners = ['requestValidationData', 'pmgi-session-updated' => 'handleSessionUpdate'];
    ...
    protected function perakuanRules()
    {
        return [
    ...
    public function requestValidationData($source)
    {
        // Only respond if this component matches the source
        if ($source === 'pmc') {
            // Prepare data to validate
            $data = [
                'exitFlag' => $this->exitFlag,
                'exitTypeFlag' => $this->exitTypeFlag,
                'comment' => $this->comment,
            ];
            // Send data and rules back to Perakuan component
            $this->dispatch('provideValidationData', 
                data: $data,
                rules: $this->perakuanRules()
            );
        }
    }
