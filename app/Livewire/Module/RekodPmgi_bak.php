...
class RekodPmgi extends Component
{
    ...
    public function streamRekodPmgi($sessionId)
    {
        ...
        //             $tempAttachmentFiles[] = $tempPath; // Track for cleanup
        //             $attachmentExtension['pym_attachment'] = pathinfo($attachmentPaths['pym_attachment'], PATHINFO_EXTENSION);
        //         }
        ...
        //             $sessionId, 
        //             $attachmentPaths, 
        //             $attachmentExtension['pym_attachment'],
        //             $imageExtensions
        //         );
        //         $directorySessionId = str_replace('/', '-', $sessionId);
        //         $folderPath = storage_path('app/public/' . $directorySessionId);
        //         $pdfImageOutput = 'PYM_'.$sessionInfo->pym_id.'_'.$directorySessionId;

        //         // $this->pdfToImageService->generate(1, $numberOfPages, $folderPath, $attachmentExtension['pym_attachment'], $pdfImageOutput);
        //     }
        // }
        ...
        if ($pymInfo && $pymInfo->attachment) {
            ...
            if (file_exists($originalPath)) {
                ...
                if ($extension === 'pdf') {
                    // Force use of Ghostscript binary
                    // Ghostscript::setGsPath('C:\\Program Files\\gs\\gs10.06.0\\bin\\gswin64c.exe');
                    // $ghostscript = new \GhostScript();
                    // $ghostscript->setGsPath('C:\\Program Files\\gs\\gs10.06.0\\bin\\gswin64c.exe');
                    
                    // Convert PDF to images
                    $pdf = new PdfToImagePdf($attachmentPaths['pym_attachment']);
                    $numberOfPages = $pdf->pageCount();
                    
                    $pdfImages = [];
                    for ($i = 1; $i <= $numberOfPages; $i++) {
                        $outputPath = storage_path('app/temp/pdf_page_' . $sessionId . '_' . $i . '.jpg');
                        $pdf->selectPage($i)->save($outputPath);
                        $pdfImages[] = asset('storage/temp/pdf_page_' . $sessionId . '_' . $i . '.jpg');
                    }
                    
                    $attachmentPaths['pym_attachment'] = $pdfImages;
                    $attachmentExtension['pym_attachment'] = 'pdf_converted';
                } else {
    ...
    private function processAttachment($attachmentPath, $key, $sessionId, &$attachmentPaths, &$attachmentExtension, $imageExtensions)
    {
        ...
        if ($extension === 'pdf') {
            try {
                ...
                $imagick = new \Imagick();
                $imagick->setResolution(150, 150); // Set before reading
                $imagick->readImage($filePath);

                dd($filePath);

                dd($imagick->readImage($filePath));
                ...
                foreach ($imagick as $pageIndex => $page) {
                    ...
                    $filename = 'pdf_' . $sessionId . '_' . $key . '_page_' . ($pageIndex + 1) . '.jpg';
                    $outputPath = $tempDir . '/' . $filename;
                    
                    $page->writeImage($outputPath);
                    $pdfImages[] = asset('storage/temp/' . $filename);
                }
                
                $imagick->clear();
                $imagick->destroy();
                
                $attachmentPaths[$key] = $pdfImages;
                $attachmentExtension[$key] = 'pdf_converted';
                
                Log::info("Successfully converted {$numberOfPages} PDF pages");
                
            } catch (\Exception $e) {
                Log::error('PDF conversion failed: ' . $e->getMessage());
                
                $attachmentPaths[$key] = route('stream.attachment', ['path' => encrypt($attachmentPath)]);
                $attachmentExtension[$key] = 'pdf_failed';
            }
        } else if (in_array($extension, $imageExtensions)) {
            $attachmentPaths[$key] = route('stream.attachment', ['path' => encrypt($attachmentPath)]);
            $attachmentExtension[$key] = $extension;
        }
    }
