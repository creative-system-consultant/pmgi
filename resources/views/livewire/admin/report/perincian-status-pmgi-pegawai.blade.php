<div class="ml-4">
    <h2 class="mt-6 text-2xl font-semibold text-gray-800 mb-4 border-b border-gray-300 pb-2 dark:text-gray-100">
        Laporan Perincian Status PMG-i Pegawai Mengikut Negeri & Cawangan
    </h2>

    <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-inner mt-6 text-gray-900 dark:text-gray-100">
        <div class="grid grid-cols-10 gap-y-2 gap-x-4 mb-4 lg:grid-cols-8">
            <!-- Report Generate Date -->
            <div class="z-50 col-span-2">
                <x-datetime-picker label="Tarikh Jana Senarai" placeholder="Tarikh Janaan" display-format="DD-MM-YYYY" wire:model="report_date_generated" without-time disabled/>
            </div>
        </div>
        ...
        <table class="w-full border border-gray-200 dark:border-gray-700 rounded-lg text-sm">
            ...
            @forelse ($results as $item)
                ...
                <td class="py-2 px-4 text-left border border-gray-300 dark:border-gray-700">{{ $loop->iteration}}</td>
                ...
                <td class="py-2 px-4 text-left border border-gray-300 dark:border-gray-700">{{ $item->pyd_name }}</td>
                <td class="py-2 px-4 text-left border border-gray-300 dark:border-gray-700">{{ $item->pym_name }}</td>
                <td class="py-2 px-4 text-right border border-gray-300 dark:border-gray-700">{{ $item->tarikh_penilaian }}</td>
                <td class="py-2 px-4 text-right border border-gray-300 dark:border-gray-700">{{ $item->status_penilaian }}</td>
                </tr>
            @empty
                <tr>
