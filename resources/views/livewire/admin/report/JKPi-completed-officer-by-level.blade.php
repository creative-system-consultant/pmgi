<div class="ml-4">
  <h2 class="mt-6 text-2xl font-semibold text-gray-800 mb-4 border-b border-gray-300 pb-2 dark:text-gray-100">
    Laporan Senarai Pegawai Selesai JKPi Mengikut Peringkat
  </h2>
  ...
  <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-inner mt-6 text-gray-900 dark:text-gray-100">
      <form wire:submit.prevent="searchFilter" class="flex flex-col sm:flex-row items-center gap-4 sm:gap-2 mb-4">
        ...
        <input type="text"
               ...
               class="px-2 py-2 border w-56 border-gray-300 rounded-md shadow-sm" />            
        ...
        <button type="button" wire:click='resetSearch';
                class="ml-4 px-3 py-2 w-60 text-gray-800 bg-gray-200 rounded-md hover:bg-gray-300">
            Set Semula Carian
        </button>
      </form>
          ...
          <tbody class="text-sm text-gray-700 dark:text-gray-200">
            @forelse ($report as $item)
              <tr class="text-xs hover:bg-gray-50 dark:hover:bg-gray-800/70">
                <td class="py-2 px-4 text-left border border-gray-300 dark:border-gray-700">{{ $item->bankOfficer->officer_name}}</td>
                <td class="py-2 px-4 text-left border border-gray-300 dark:border-gray-700">{{ $item->state->description }}</td>
                <td class="py-2 px-4 text-left border border-gray-300 dark:border-gray-700">{{ $item->branch->branch_name }}</td>
                <td class="py-2 px-4 text-left border border-gray-300 dark:border-gray-700">{{ $item->level->pmgi_level_desc}}</td> 
