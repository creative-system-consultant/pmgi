<div class="ml-4">
    <h2 class="mt-6 text-2xl font-semibold text-gray-800 mb-4 border-b border-gray-300 pb-2 dark:text-gray-100">
        Laporan Ringkasan Bil PYD Mengikut Negeri & Cawangan
    </h2>
    ...
    <div class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-inner mt-6 text-gray-900 dark:text-gray-100">
        ...
        <table class="w-full border border-gray-200 dark:border-gray-700 rounded-lg text-sm">
            ...
            @forelse ($results as $item)
                ...
                <td class="py-2 px-4 text-right border border-gray-300 dark:border-gray-700">
                    {!! $item->bil_pyd > 0 ? '<a href="' . route('report.admin.perincian_status', 
                        [
                            'start'     => $report_start_date,
                            'end'       => $report_end_date,
                            'branch'    => $item->kod_cawangan,  // use the actual SP column name (code, not description)
                            'peringkat' => $item->peringkat,
                        ]) . '" class="cursor text-blue-600 underline">' . $item->bil_pyd . '</a>' : $item->bil_pyd 
                    !!}
                </td>
                <td class="py-2 px-4 text-right border border-gray-300 dark:border-gray-700">{{ $item->bil_pyd_selesai }}</td>
                <td class="py-2 px-4 text-right border border-gray-300 dark:border-gray-700">{{ $item->baki_belum_selesai_pyd }}</td>
                </tr>
            @empty
                <tr>
